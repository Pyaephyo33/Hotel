                ...
                <form action="{{ url('admin/search-roles') }}" class="float-right" method="GET">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex space-x-2 mx-2">
                            ...
                            <a href="{{url('admin/roles/create')}}" class="flex items-center btn btn-outline btn-sm btn-primary hover:text-white">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="flex space-x-2">
                    ...
                    <a href="#" onclick="document.getElementById('deleteModal{{ $role->id }}').showModal()">
                    ...
                    <a href="{{ url('admin/roles/permissions/assign/' . $role->id) }}">
                  ...
                  {{-- Delete Modal --}}
                  <dialog id="deleteModal{{ $role->id }}" class="modal">
                    <div class="modal-box">
                        <h3 class="font-bold text-lg text-yellow-600">Delete Confirm!</h3>
                        <p class="py-4">Are you sure you want to delete?</p>
                        <div class="modal-action">
                            <form id="deleteForm{{ $role->id }}" action="{{ url('admin/roles/delete/' . $role->id) }}" method="post">
                                @csrf
                                @method('delete')
                                <button type="button" onclick="submitDeleteForm('{{ $role->id }}')" class="btn btn-error btn-sm">Delete</button>
                                <button type="button" onclick="document.getElementById('deleteModal{{ $role->id }}').close()" class="btn btn-sm">Close</button>
                            </form>
                        </div>
                    </div>
                  </dialog>
