        ...
        {{-- Form Head --}}
            <div class="grid grid-cols-6 gap-4 mb-3">
                ...
                <div class="col-end-9 col-span-2">
                    <a href="{{url('admin/foods')}}" class="flex items-center btn btn-outline btn-sm hover:text-white">
            ...
            {{-- Form --}}
            @if(empty($food->id))
            <form action="{{url('admin/foods')}}" method="post">
                @else
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <div class="label">
                      <span class="label-text-2xl">Food</span>
                    </div>
                    <input type="text" placeholder="Enter Name" class="input input-bordered w-full @error('name') input-error @enderror" id="name" name="name" value="{{old('name') ?? $food->name}}" />
                    @error('name')
                ...
                <div>
                    ...
                    <input type="number" placeholder="Enter Price" class="input input-bordered w-full @error('price') input-error @enderror" id="price" name="price" value="{{old('price') ?? $food->price}}" />
                    @error('price')
                        <span class="text-sm text-red-500">{{ $message }}</span>
                    @enderror
                  </label>
           ...
           <div class="grid justify-items-end mt-5">
                <div class="flex space-x-2">
                    ...
                    @else
                        <a href="{{url('admin/foods')}}" class="btn btn-active btn-sm">Cancel</a>
