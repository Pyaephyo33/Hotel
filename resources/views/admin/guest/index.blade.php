    ...
    <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
        ...
        <div class="grid grid-cols-6 gap-4 mb-5">
            ...
            <div class="col-end-7 col-span-2">
                <form action="{{url('admin/search-guests')}}" class="float-right" method="GET">
                    @csrf
                    <div class="flex items-center">
                        <input type="text" name="search" class="input input-bordered btn-sm w-full" placeholder="Search">
                        <div class="flex space-x-2 mx-2">
                            <button type="submit" class="btn btn-outline btn-sm">Search</button>
                            <a href="{{url('admin/guests/create')}}" class="flex items-center btn btn-outline btn-sm btn-primary hover:text-white">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="flex space-x-2">
                    <a href="{{url('admin/guests/'.$guest->id.'/edit')}}">
                    ...
                    <a href="#" onclick="document.getElementById('deleteModal{{ $guest->id }}').showModal()">
                  ...
                  {{-- Delete Modal --}}
                  <dialog id="deleteModal{{ $guest->id }}" class="modal">
                    <div class="modal-box">
                        <h3 class="font-bold text-lg text-yellow-600">Delete Confirm!</h3>
                        <p class="py-4">Are you sure you want to delete?</p>
                        <div class="modal-action">
                            <form id="deleteForm{{ $guest->id }}" action="{{ url('admin/guests/' . $guest->id) }}" method="post">
                                @csrf
                                @method('delete')
                                <button type="button" onclick="submitDeleteForm('{{ $guest->id }}')" class="btn btn-error btn-sm">Delete</button>
                                <button type="button" onclick="document.getElementById('deleteModal{{ $guest->id }}').close()" class="btn btn-sm">Close</button>
                            </form>
                        </div>
                    </div>
                  </dialog>
